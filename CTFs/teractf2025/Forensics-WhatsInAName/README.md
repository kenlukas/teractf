...
First, `tshark` opens the pcap file, and the -Y displays DNS packets that are not responses, and shows the query name.  I pipe that into `grep` because there are queries to other domains and I only want the workisfxd ones.  I pipe that output into `cut` to show the first field before the period.  Then I use xargs to echo them without a newline.  This gives me what appears to be a base64 string.

```sh
% tshark -r forensic.pcap -Y "dns.flags.response eq 0" -T fields -e dns.qry.name |grep workisfxd | \
cut -d. -f1 |xargs -I {} echo -n {}
